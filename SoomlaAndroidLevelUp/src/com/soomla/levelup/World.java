...
public class World {
     ...
     *
     * @param worldId the world's ID
     * @param isSingleScore determines if the world has only one score
     */
    public World(String worldId, boolean isSingleScore) {
        this(worldId, null, new HashMap<String, World>(), new HashMap<String, Score>(), new ArrayList<Challenge>());
    ...
    public World(String worldId, int numLevels, boolean innerLevelsSingleScore) {
        this(worldId, false);
        for (int i = 0; i < numLevels; i++) {
            Level aLvl = new Level(idForAutoGeneratedLevel(i), innerLevelsSingleScore);
            this.addInnerWorld(aLvl);
        }
    ...
    public String idForAutoGeneratedLevel(int idx) {
        return this.mWorldId + "_level" + idx;
    }
    ...
    public void singleScore() {
        ...
        Score oneScore = new Score(this.mWorldId + "_oneScore");
        this.addScore(oneScore);
    }

    public void decScores(double amount) {
        if (this.mScores == null || this.mScores.isEmpty()) {
            SoomlaUtils.LogError(TAG, "(decScores) You don't have any scores defined in this world. worldId: " + this.mWorldId);
            return;
        }

        for (Score score : this.mScores.values()) {
            decScore(score.getScoreId(), amount);
        }
    }

    public void decScore(String scoreId, double amount) {
        this.mScores.get(scoreId).dec(amount);
    }

    public void incScores(double amount) {
        if (this.mScores == null || this.mScores.isEmpty()) {
            SoomlaUtils.LogError(TAG, "(incScores) You don't have any scores defined in this world. worldId: " + this.mWorldId);
            return;
        }

        for (Score score : this.mScores.values()) {
            incScore(score.getScoreId(), amount);
        }
    }

    public void incScore(String scoreId, double amount) {
        this.mScores.get(scoreId).inc(amount);
    }
